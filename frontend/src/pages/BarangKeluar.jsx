...
export default function BarangKeluar() {
  ...
  const historyKeluar = transactions.filter(tx => tx.type === "out" || tx.type === "return");

  return (
    <div className="space-y-6 pb-10">
      {/* HEADER PAGE */}
      <div>
        <h1 className="text-2xl font-bold text-gray-800 tracking-tight"> Pencatatan Barang Keluar & Penjualan</h1>
        <p className="text-xs text-gray-400 mt-0.5">Catat pengurangan persediaan barang untuk penjualan atau retur pelanggan</p>
      </div>

      {/* FORM INPUT BARANG KELUAR (LUAS TANPA KOTAK DETAIL SAMPING) */}
      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-xs">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">Nama Pembeli / Tujuan</label>
              <input 
                type="text" 
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">Tipe Pengeluaran</label>
              <select 
              ...
              >
                <option value="out">Barang Keluar (Penjualan / Nota)</option>
                <option value="return">Retur Keluar (Barang Rusak / Expired)</option>
              </select>
            </div>
          </div>
            ...
            >
              ...
              {products.map(p => (
                <option key={p.id} value={p.id}>
                  {p.name} (Sisa Stok Tersedia: {p.stock} Unit)
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="flex items-center justify-between bg-gray-50 p-3 rounded-xl border border-gray-200">
            <div className="flex flex-col">
              <span className="text-sm font-bold text-gray-700">Jumlah Keluar</span>
              {selectedProduct && (
                <span className="text-[11px] text-gray-400 font-medium mt-0.5">Stok maksimal: {selectedProduct.stock} unit</span>
              )}
            </div>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-400 mb-1">Catatan Dokumen</label>
            <textarea 
              rows={2} 
              placeholder="Keterangan pengiriman atau keperluan barang keluar harian..." 
              className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white text-sm text-gray-700 placeholder-gray-400 transition-all" 
              value={note} 
      ...
      {/* TABEL LAPORAN MULTI-COLUMN PREMIUM */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-xs p-2">
        <div className="px-4 py-3 border-b border-gray-50">
          <h3 className="text-sm font-bold text-gray-700">Laporan Riwayat Mutasi Keluar</h3>
        </div>
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="text-xs uppercase bg-gray-50 text-gray-400 font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 rounded-r-xl">Keterangan Riwayat</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {historyKeluar.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-xs text-gray-400 italic">
                    Belum ada rekaman data transaksi barang keluar harian.
                  </td>
                </tr>
              ) : (
                historyKeluar.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-gray-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4 font-black text-rose-600">-{tx.qty} Unit</td>
